...
function getBody(gt, challenge, slider, get_c, get_s, h9s9) {
  _tkts = {}
  function timing() {
    var now = Date.now()
    var tim = {
  ...
  var window = {
    ...
    "performance": {
      "timing": {
        "connectStart": 1683189097709,
        "navigationStart": 1683189097700,
        "secureConnectionStart": 0,
        "fetchStart": 1683189097709,
        "domContentLoadedEventStart": 1683189098273,
        "responseStart": 1683189098116,
        "domInteractive": 1683189098273,
        "domainLookupEnd": 1683189097709,
        "responseEnd": 1683189098117,
        "redirectStart": 0,
        "requestStart": 1683189097744,
        "unloadEventEnd": 1683189098122,
        "unloadEventStart": 1683189098122,
        "domLoading": 1683189098123,
        "domComplete": 1683189098275,
        "domainLookupStart": 1683189097709,
        "loadEventStart": 1683189098275,
        "domContentLoadedEventEnd": 1683189098275,
        "loadEventEnd": 1683189098278,
        "redirectEnd": 0,
        "connectEnd": 1683189097709
      }
    }
  }
  ht = window["navigator"];
  ...
  const passtime = slider[slider.length - 1][2]
    , slider_len = slider[slider.length - 1][0] - 1
  o = {
    "lang": "zh-cn",
    "userresponse": H(slider_len, challenge),
    "passtime": passtime,
    "imgload": 111,
    "aa": $_BBEI($_FDL(slider), get_c, get_s),
    "ep": get_ep(),
    "h9s9": h9s9,
    "rp": X(gt + challenge.slice(0, 32) + passtime)
  }
  let u = $_CCDf()
  let l = aesV(JSON.stringify(o), $_CCEV())
  let h = m['$_FEE'](l)
  ...
  let body = {
    ...
    "client_type": "web",
    "w": h + u,
    "callback": "geetest_" + new Date().getTime()
  }
  return body;
}
...
async function geth9s9(gct_path) {
  const gct_js = await (await fetch(gct_path)).text();
  const function_name = gct_js.match(/\)\)\{return (.*?)\(/)[1];
  const break_position = gct_js.indexOf("return function(t){");
  const gct_js_new = "window = global;" + gct_js.slice(0, break_position) + "window.gct=" + function_name + ";" + gct_js.slice(break_position);
  const gct = new Function(gct_js_new + `
  function getGct(){
...
async function getPicPos(bg, fullbg) {
  //返回{x,y}
  let bgImg = await sharp(bg).raw().toBuffer();
  let fullbgImg = await sharp(fullbg).raw().toBuffer();
  let colorGap = 170;
  let colorGap2 = 20;
  for (let i = 60; i < 260; i++) {
    for (let j = 0; j < 160; j++) {
      const bgImgRGB = [bgImg[(i + j * 260) * 4] + bgImg[(i + j * 260) * 4 + 1] + bgImg[(i + j * 260) * 4 + 2]];
      const fullbgImgRGB = [fullbgImg[(i + j * 260) * 4] + fullbgImg[(i + j * 260) * 4 + 1] + fullbgImg[(i + j * 260) * 4 + 2]];
      if (
        Math.abs(fullbgImgRGB - bgImgRGB) > colorGap
      ) {
        //为了避免误识别到之前拼接图片导致的色差，这里再判断往右边和往下15*10的像素是否也有色差
        let flag = true;
        for (let m = 0; m < 10; m++) {
          for (let n = 0; n < 10; n++) {
            if (
              Math.abs(fullbgImgRGB - bgImgRGB) < colorGap2
            ) {
              flag = false;
              break;
            }
...
async function get_validate(gt, challenge) {
  const codeTypeUrl = `https://apiv6.geetest.com/ajax.php?gt=${gt}&challenge=${challenge}&lang=zh-cn&pt=0&client_type=web&w=&callback=geetest_${Date.now()}`;
  let response = await (await fetch(codeTypeUrl)).text()
  //geetest_1683022722169({"status": "success", "data": {"result": "slide"}})
  const codeType = JSON.parse(response.match(/{.*?}\)/g)[0].replace(")", "")).data.result;
  if (codeType == "slide") {
    const codeUrl = `https://apiv6.geetest.com/get.php?is_next=true&type=slide3&gt=${gt}&challenge=${challenge}&lang=zh-cn&https=true&protocol=https%3A%2F%2F&offline=false&product=embed&api_server=apiv6.geetest.com&isPC=true&autoReset=true&width=100%25&callback=geetest_${Date.now()}`;
    response = await (await fetch(codeUrl)).text();
    //geetest_1683022596073({"gt":"d019a22590a54475b8e30eeb2854aab9","challenge":"c74d03b470e0ace1bf10bd8fd112f086c8","id":"ac74d03b470e0ace1bf10bd8fd112f086","bg":"pictures/gt/2139ddc2a/bg/c116a80df.jpg","fullbg":"pictures/gt/2139ddc2a/2139ddc2a.jpg","link":"","ypos":47,"xpos":0,"height":160,"slice":"pictures/gt/2139ddc2a/slice/c116a80df.png","api_server":"https://apiv6.geetest.com","static_servers":["static.geetest.com/","dn-staticdown.qbox.me/"],"mobile":true,"theme":"ant","theme_version":"1.2.6","template":"","logo":false,"clean":false,"type":"multilink","fullpage":false,"feedback":"","show_delay":250,"hide_delay":800,"benchmark":false,"version":"6.0.9","product":"embed","https":true,"width":"100%","c":[12,58,98,36,43,95,62,15,12],"s":"7074475f","so":0,"i18n_labels":{"cancel":"取消","close":"关闭验证","error":"请重试","fail":"请正确拼合图像","feedback":"帮助反馈","forbidden":"怪物吃了拼图，请重试","loading":"加载中...","logo":"由极验提供技术支持","read_reversed":false,"refresh":"刷新验证","slide":"拖动滑块完成拼图","success":"sec 秒的速度超过 score% 的用户","tip":"请完成下方验证","voice":"视觉障碍"},"gct_path":"/static/js/gct.b71a9027509bc6bcfef9fc6a196424f5.js"})
    const codeInfo = JSON.parse(response.match(/{.*?}\)/g)[0].replace(")", ""));
    let bg = "https://static.geetest.com/" + codeInfo.bg;
    let fullbg = "https://static.geetest.com/" + codeInfo.fullbg;
    let slice = "https://static.geetest.com/" + codeInfo.slice;
    const gct_path = "https://static.geetest.com" + codeInfo.gct_path;
    const c = codeInfo.c;
    const s = codeInfo.s;
    const challenge2 = codeInfo.challenge;
    const h9s9 = (await geth9s9(gct_path)).h9s9;
    bg = await restore_picture(bg);
    fullbg = await restore_picture(fullbg);
    slice = await fetch(slice).then(async (res) => Buffer.from(await res.arrayBuffer()));
    let gapPos = await getPicPos(bg, fullbg);
    // console.log(gapPos);
    // // fs.writeFileSync("ss.txt",JSON.stringify(gapPos));
    // // return
    // //gapPos可能是多边形,只保留正方形并取出左上角位置
    // fs.writeFileSync("bg.png", bg);
    // fs.writeFileSync("fullbg.png", fullbg);
    // //根据gapPos把slice拼接到bg上写入到文件
    // const ss = sharp(bg);
    // const l_ = ss.composite([{ input: slice, left: gapPos.x, top: gapPos.y }]);
    // await l_.toFile("l.png");
    let slide_num = 6;
    let message, res
    for (let sln = 0; sln < slide_num; sln++) {
      const slide_track = gapPos.x - sln
      const slide_list = get_slide_track(slide_track);
      let body = getBody(gt, challenge2, slide_list, c, s, h9s9)
      body = Object.keys(body).map((key) => `${key}=${body[key]}`).join("&");
      const submitUrl = "https://apiv6.geetest.com/ajax.php?" + body;
      response = await (await fetch(submitUrl)).text();
      res = JSON.parse(response.match(/{.*?}\)/g)[0].replace(")", ""));
      message = res.message;
      if (message != "fail") {
        break;
      }
...
下面获取验证码然后调用过滑块
*/
const infoUrl = "https://bbs-api.mihoyo.com/misc/api/createVerification?is_high=false";
fetch(infoUrl).then(async (res) => {
  //{"retcode":0,"message":"OK","data":{"challenge":"aa6940bf801d53d31f956c3f99e8f82b","gt":"d019a22590a54475b8e30eeb2854aab9","new_captcha":1,"success":1}}
  const info = await res.json();
  const challenge = info.data.challenge;
  const gt = info.data.gt;
  console.log(await get_validate(gt, challenge));
}).catch((err) => {
  console.log(err);
});
